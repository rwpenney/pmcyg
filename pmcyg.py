...
class PMbuilder(object):
    ...
    def _buildSetupFiles(self, header, pkgdict, packages):
        ...
        spath = os.path.join(self._tgt_dir, 'setup.ini')
        fp = open(spath, 'w+t')
        ...
        fp.seek(0)
        cpsr = bz2.BZ2File(os.path.join(self._tgt_dir, 'setup.bz2'), mode='w')
        cpsr.write(fp.read())
        cpsr.close()
        fp.close()
        ...
        try:
            urllib.urlretrieve(self._exesrc, os.path.join(self._tgt_dir, 'setup.exe'))
        except Exception, ex:
            raise PMCygException, "Failed to retrieve %s\n - %s" % ( self._exesrc, str(ex) )

        hp = open(os.path.join(self._tgt_dir, 'md5.sum'), 'wt')
        for fl in ['setup.ini', 'setup.bz2', 'setup.exe']:
            hshr = md5hasher()
            fp = open(os.path.join(self._tgt_dir, fl), 'rb')
            hshr.update(fp.read())
            fp.close()
            hp.write('%s  %s\n' % ( hshr.hexdigest(), fl ))
        hp.close()
    ...
    def _doDownloading(self, header, pkgdict, packages, dummy=False):
        ...
        if not dummy:
            if not os.path.isdir(self._tgt_dir):
                os.makedirs(self._tgt_dir)
        ...
        for (pkgfile, pkgsize, pkghash) in downloads:
            ...
            tgtpath = os.path.join(self._tgt_dir, pkgfile)
...
class GUIthread(threading.Thread):
    ...
    def download(self):
        ...
        if self.parent.pkgfiles:
            usrpkgs = self.builder.ReadPackageLists(self.parent.pkgfiles)

        print 'Building mirror from %s...' % self.parent.builder.GetIniURL()
        self.parent.builder.BuildMirror(usrpkgs)
...
def PlainMain(builder, pkgfiles, allpkgs=False, dummy=False):
    ...
    try:
        builder.BuildMirror(usrpkgs, include_all=include_all, dummy=dummy)
