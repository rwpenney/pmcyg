...
http://mirror.mcs.anl.gov/cygwin/;mirror.mcs.anl.gov;United States;Illinois
    ...
    def MakeTemplate(self, stream):
        ...
        print >>stream, '# Autogenerated by pmcyg on %s\n# from: %s' % ( time.asctime(), self.GetIniURL() )
...
This is free software, and you are welcome to redistribute it under the terms of the GNU General Public License (v3).""" % ( PMCYG_VERSION ))
    ...
    def setMirror(self, mirror):
        ...
        if mirror != self.builder.GetMirrorURL():
            self.builder.SetMirrorURL(mirror)
...
def main():
    ...
    parser.add_option('--generate-template', '-g', type='string', default=None,
    ...
    if opts.generate_template:
        fp = open(opts.generate_template, 'wt')
