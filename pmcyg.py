...
class PMbuilder(object):
    ...
    def UpdatePackageLists(self, filenames, bckp=".orig"):
        ...
        as that generated by PackageList.MakeTemplate()."""
...
class PackageSet(object):
    ...
    def extend(self, pkgs):
        # FIXME - construct union of arch constraints etc.
        if isinstance(pkgs, PackageSet):
            self._pkgs.update(pkgs._pkgs)
        else:
            for p in pkgs:
                if not p in self._pkgs:
                    self._pkgs[p] = {}

    def extract(self, **kwargs):
        # FIXME - apply arch constraints
        return list(self._pkgs.iterkeys())

    def _ingestFile(self, fname):
        """Parse a text file containing a list of Cygwin package names,
        one per line."""

        # FIXME - extend this to include architecture switches
